...
- **内置完整 RAG**：文档加载（可选 Tika 解析 PDF/Word/Excel，或 MinerU 云端解析扫描件/公式/复杂版面为 Markdown）、切块、八大向量库适配（Pinecone / Qdrant / pgvector / Milvus / Redis / Elasticsearch / Chroma / InMemory）、混合检索、重排、引用标注，整条链路在 SDK 内实现，无需外挂检索框架。
