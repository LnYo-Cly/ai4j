...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 本轮范围清晰，采用 coordinator self-review；人工确认由 harness review queue 承接。 | 不调用 reviewer subagent。 |
| Would a worker subagent materially help? | no | 改动横跨共享 API、coding、CLI、docs 和治理表，拆 worker 反而增加集成风险。 | coordinator 单线执行。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-09 04:30 | n/a | n/a | 本轮不需要 worker subagent。 |
...
| Subagent 模式 | none | 小范围跨模块协调由 coordinator 单线完成。 |
| 审查模型 | self-check + human review queue | 代码行为由 targeted tests 覆盖，人工确认通过 harness 后续完成。 |
| Worktree 策略 | same checkout | 无 worker 写入，不需要独立 worktree。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1 + L2 | targeted Java tests 覆盖行为，package/docs gates 覆盖集成面。 |
...
| L1 | extension-api / coding / CLI targeted JUnit | `progress.md` | 相关测试通过 |
| L2 | `mvn -DskipTests package`; docs-site typecheck/build | `progress.md` | package 和 docs build 通过 |
| L3 | 不适用 | `review.md` 与 walkthrough | 本轮不涉及 live provider 或发布凭证 |
