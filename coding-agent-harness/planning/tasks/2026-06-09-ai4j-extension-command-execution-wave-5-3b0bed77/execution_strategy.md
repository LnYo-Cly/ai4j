...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 本轮代码切片集中，使用 self adversarial review 足够；人工确认仍走 harness review workbench。 | 在 `review.md` 记录 self review。 |
| Would a worker subagent materially help? | no | 修改范围跨 CLI、docs-site 和治理 SSoT，拆 worker 会增加协调成本；当前由 coordinator 串行完成更稳。 | 不使用 worker。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-09 | CLI command execution Wave 5 | `main` | 不需要 worker 授权。 |
...
| Subagent 模式 | none | 范围集中，无需 worker。 |
| 审查模型 | self-check + adversarial review | 重点检查第三方代码执行门禁、参数解析和回归证据。 |
| Worktree 策略 | same checkout | 当前 `main` 串行执行，任务开始时无未提交用户改动。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1 + L2 | CLI targeted tests + monorepo package/docs-site smoke。 |
...
| L1 | `mvn -pl ai4j-cli -am -Dtest=Ai4jCliTest -DfailIfNoTests=false -DskipTests=false test` | `progress.md` | CLI targeted tests pass |
| L2 | `mvn -DskipTests package` | `progress.md` | 10 reactor modules package |
| L2 | docs-site typecheck/build | `progress.md` | plugin docs page compiles |
