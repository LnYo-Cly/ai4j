...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F-001 | P3 | 初始任务名容易让人误以为要新增 cost runtime；实际 `origin/main` 已有 trace pricing resolver 和 metrics cost 字段。 | `TracePricing`, `TracePricingResolver`, `AgentTraceListener.metricsFromUsage`, `AgentTraceListenerTest`, `LangfuseTraceExporterTest` | 不新增重复 abstraction；补 docs usage 并验证既有路径。 | no | documented | no | 如用户明确要 `AgentResult` 直接拿 cost，另开 API 设计任务。 |
