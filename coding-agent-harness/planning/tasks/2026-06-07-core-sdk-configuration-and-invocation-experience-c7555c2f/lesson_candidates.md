...
| Closeout token | checked-none: design-wave-routing-local |
...
本任务产出的是 task-local 升级设计，结论已写入 `design.md` 和 `findings.md`。是否把“配置 helper 优先于 Chat facade”沉淀成全局标准，需要后续人工确认后单独创建 lesson/reference 任务。
