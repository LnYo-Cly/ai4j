...
> 记录时间：2026-06-20 18:26  
> 状态来源：实现 worktree 已有 dirty diff，继续从验证/治理/收口阶段推进，不重复实现。

当前实现已经完成 `/memory` 主要代码、测试和 docs-site 说明。剩余计划按以下顺序执行：

1. 修复 `docs-site/docs/coding-agent/command-reference.md` 的 trailing whitespace，重新跑 `git diff --check`。
2. 将 `/memory` 纳入固定 CLI slash command parity regression：同步 `docs/05-TEST-QA/Regression-SSoT.md` 与 `docs/05-TEST-QA/Cadence-Ledger.md`，明确 root completion、runtime dispatch、ACP command、help/palette、formatter、docs reference 必须一致。
3. 更新任务本地 `progress.md`、`review.md`、`walkthrough.md`、`lesson_candidates.md`：把 targeted CLI tests、broad CLI tests、docs-site build、secret scan、diff check、Harness status 作为证据记录。
4. 重新验证：`git diff --check`、`npx --yes coding-agent-harness status --json .`；如需要提交前再复跑 targeted CLI tests。
5. 提交范围只包含本任务代码、docs-site、Regression/Cadence 和 task package；不得提交 `docs-site/node_modules` 或其它 ignored/generated 依赖。
6. 提交后执行 `task-review`，如 lifecycle CLI 生成治理变更，单独提交。
7. 推送 `feature/cli-memory-compact-ux`，创建 PR 到 `dev`，等待/检查 CI；CI 绿后再合并。

当前明确不做：

- 不新增 memory editor；
- 不打印 raw memory / prompt / tool output；
- 不修改 `ai4j-coding` public API；
- 不把 auto-compact breaker 不可取字段作为阻塞项。
