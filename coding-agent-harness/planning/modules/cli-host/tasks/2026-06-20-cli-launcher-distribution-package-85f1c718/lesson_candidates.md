# CLI launcher distribution package - 教训候选
...
| Closeout token | checked-none:cli-distribution-bounded-slice |
| Source task | 2026-06-20-cli-launcher-distribution-package-85f1c718 |
| Owner | coordinator |
...
本任务发现的主要经验是根 `.gitignore` 的通用 `dist/` 规则会误伤 `ai4j-cli/src/main/dist`。这属于本仓库本次发行包布局的局部实现细节，已经在 `.gitignore` 精确反忽略、`findings.md` 和 package layout test 中处理，不需要沉淀为跨项目 Harness lesson。
