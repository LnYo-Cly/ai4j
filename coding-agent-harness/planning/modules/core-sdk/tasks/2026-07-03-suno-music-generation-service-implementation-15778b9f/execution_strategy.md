...
| worker subagent | not authorized | write only after user approval | n/a | n/a | n/a | n/a | not needed; implementation was single-owner and shared regression/task docs stayed with coordinator |
...
| 审查模型 | self-check | ChatFire Suno `/suno/...` contract 由 MockWebServer 和 module/starter gates 覆盖；live provider 是 opt-in residual。 |
| Worktree 策略 | dedicated worktree | 使用 `G:\My_Project\java\ai4j-sdk\.worktrees\feature\suno-music-service` / `feature/suno-music-service`。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1/L2 | L1 覆盖 core/starter tests；L2 package smoke 覆盖跨模块编译打包。 |
...
| --- | --- | --- | --- | --- |
| n/a | C-001..C-006; task diff | n/a | coordinator self-review in `review.md` | coordinator |
...
| L1 | `mvn -pl ai4j "-Dtest=SunoMusicServiceTest,AiServiceRegistryTest" -DskipTests=false test`; `mvn -pl ai4j -am -DskipTests=false test`; `mvn -pl ai4j-spring-boot-starter -am -DskipTests=false test` | `progress.md` and `review.md` | all commands pass |
| L2 | `mvn -DskipTests package` | `progress.md` and `review.md` | 11 reactor projects package successfully |
| L3 | live ChatFire Suno smoke with `CHATFIRE_API_KEY` | `review.md` and walkthrough if later approved | opt-in only; not a blocker for this task |
...
- 真实 Suno 生成需要 provider key、余额或可能产生费用时，停止并转为 LV-001 opt-in residual。
