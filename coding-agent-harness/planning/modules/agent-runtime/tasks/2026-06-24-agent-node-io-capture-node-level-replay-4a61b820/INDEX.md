# Agent node IO capture + node-level replay - 任务包索引
...
| Field | Value |
| --- | --- |
| Task ID | `2026-06-24-agent-node-io-capture-node-level-replay-4a61b820` |
| Budget | `complex` |
| Preset | `module` |
...
| Command Shape | harness new-task --budget complex --locale zh-CN --title 'Agent node IO capture + node-level replay' --module agent-runtime --preset module '<target>' |
| Budget | complex |
