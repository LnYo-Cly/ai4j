...
| RG-004 | CLI/TUI/ACP host | `mvn -pl ai4j-cli -am -DskipTests=false test` | `ai4j-cli/` CLI, TUI, ACP, session, rendering changes | L1-tests | 2026-06-09 remote pass | pass | project coordinator | none |
...
| RG-008 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | `docs-site/` content, config or workflow changes | L2-local-smoke | 2026-06-10 local and remote pass with package-script heap | pass | project coordinator | R-004 |
