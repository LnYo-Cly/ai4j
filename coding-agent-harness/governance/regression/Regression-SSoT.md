...
| RG-001 | core SDK module | `mvn -pl ai4j -am -DskipTests=false test` | `ai4j/` provider, protocol, RAG, vector, MCP, image, audio, realtime, agentflow connector changes | L1-tests | 2026-06-09 remote pass | pass | project coordinator | none |
| RG-002 | agent runtime module | `mvn -pl ai4j-agent -am -DskipTests=false test` | `ai4j-agent/` workflow, memory, trace, subagent/team orchestration changes | L1-tests | 2026-06-09 remote pass | pass | project coordinator | none |
| RG-003 | coding runtime module | `mvn -pl ai4j-coding -am -DskipTests=false test` | `ai4j-coding/` tools, outer-loop, checkpoint, shell/apply-patch changes | L1-tests | 2026-06-09 remote pass | pass | project coordinator | none |
| RG-004 | CLI/TUI/ACP host | `mvn -pl ai4j-cli -am -DskipTests=false test` | `ai4j-cli/` CLI, TUI, ACP, session, rendering changes | L1-tests | 2026-06-11 targeted pass, 29 `Ai4jCliTest` tests | pass | project coordinator | none |
| RG-005 | Spring Boot starter | `mvn -pl ai4j-spring-boot-starter -am -DskipTests=false test` | `ai4j-spring-boot-starter/` property binding or auto-configuration changes | L1-tests | 2026-06-09 remote pass | pass | project coordinator | none |
| RG-006 | FlowGram starter and task APIs | `mvn -pl ai4j-flowgram-spring-boot-starter -am -DskipTests=false test` | `ai4j-flowgram-spring-boot-starter/` or `ai4j-flowgram-demo/` changes | L1-tests | 2026-06-09 remote pass | pass | project coordinator | none |
| RG-007 | monorepo package build | `mvn -DskipTests package` | root `pom.xml`, `ai4j-bom/`, shared build plugins, release/publishing logic, cross-module impact | L2-local-smoke | 2026-06-11 pass, 11 reactor projects | pass | project coordinator | none |
| RG-008 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | `docs-site/` content, config or workflow changes | L2-local-smoke | 2026-06-11 local pass after extension check gate docs update | pass | project coordinator | R-004 |
| RG-009 | FlowGram webapp demo build and test | `npm run test`, `npm run lint`, `npm run ts-check`, then `npm run build` in `ai4j-flowgram-webapp-demo/` | `ai4j-flowgram-webapp-demo/` frontend changes | L2-local-smoke | 2026-06-10 local and remote pass | pass | project coordinator | none |
...
| LV-003 | FlowGram demo end-to-end behavior | backend plus web demo/manual or browser-driven scenario | FlowGram demo release or integration task | L4-browser-human-proxy | mapped-pending-runbook | pass-with-residual | project coordinator | mapped-pending-runbook |
| CR-001 | release signing and Central publishing | release profile dry run or operator-approved publish command | release candidate only | L3-live to L5-hard-gate | mapped-pending-runbook | pass-with-residual | project coordinator | none |

## 未关闭回归残余

| 残余 ID | Gate ID | 问题 | 严重级别 | 负责人 | 创建日期 | 路由 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| R-001 | RG-001..RG-007 | resolved 2026-06-09: remote Java regression run `27202972949` passed on `main@41ca7bd`; `main` and `dev` branch protection now require strict `java-regression` status check. | P1 | project coordinator | 2026-04-26 | `coding-agent-harness/planning/tasks/2026-06-09-ai4j-java-regression-ci-r-001-verification-daa85690/progress.md` | closed |
...
| R-003 | RG-009 | resolved 2026-06-10: FlowGram webapp demo `test`, `test:cov`, and `watch` scripts are no longer placeholders; `npm test` runs backend workflow normalization/serialization checks and CI runs it before lint/type/build; remote `flowgram-webapp-regression` run `27253773916` passed on `main@b0993f56`. | P2 | project coordinator | 2026-04-26 | `coding-agent-harness/planning/tasks/2026-06-10-ai4j-flowgram-webapp-real-test-gate-r-003-fix-4c2813e4/progress.md` | closed |
...
| R-005 | RG-008 | resolved 2026-06-10: `docs-site/package.json` bakes `node --max-old-space-size=8192` into `npm run typecheck` and `npm run build`; both commands pass without external `NODE_OPTIONS`, and docs-build run `27220942110` plus docs-pages run `27220942127` passed on `main@0df7094`. | P2 | project coordinator | 2026-04-27 | `coding-agent-harness/planning/tasks/2026-06-09-ai4j-docs-site-node-heap-regression-r-005-fix-08b5fbcb/progress.md` | closed |
...
| R-007 | RG-009 | resolved 2026-06-09: `.github/workflows/flowgram-webapp-regression.yml` runs the FlowGram webapp lint/type/build baseline; GitHub Actions run `27211219761` passed; `main` and `dev` branch protection now require strict `flowgram-webapp-regression` alongside `java-regression`. | P2 | project coordinator | 2026-06-04 | `coding-agent-harness/planning/tasks/2026-06-09-ai4j-flowgram-webapp-ci-r-007-fix-eaa0ecce/progress.md` | closed |
