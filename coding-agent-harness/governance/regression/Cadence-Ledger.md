...
| `ai4j-flowgram-webapp-demo/` frontend changes | RG-009 | LV-003 when paired with a real backend/demo scenario | touched-surface; PR/push workflow if configured path matches; opt-in-live for end-to-end demo | L2 plus L4 when approved | Mirrors `.github/workflows/flowgram-webapp-regression.yml`: frontend demo lint/type/build baseline plus stable aggregate `flowgram-webapp-regression` required check; browser scenario is separate evidence. | project coordinator |
...
| SRB-V2-011 | 2026-06-10 | Docs-site Node heap R-005 fix | bake 8GB Node heap into docs-site package scripts and align docs workflows with RG-008 | RG-008 local pass | pass-with-residual | `npm run typecheck` passed without external `NODE_OPTIONS`; `npm run build` passed and generated `docs-site/build`; `npx.cmd --yes yaml-lint .github/workflows/docs-build.yml .github/workflows/docs-pages.yml` passed. | R-005 closed; R-004 remains routed for independent Windows Docusaurus cleanup lock risk | Next executable or docs-site surface change. |
