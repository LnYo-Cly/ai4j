...
| SRB-V2-013 | 2026-06-11 | Extension check gate | add `ai4j-cli extension check <id> --enable ...` as validation plus requested activation gate and update plugin author/recipe docs | RG-004 targeted pass; RG-007 pass; RG-008 pass | pass | `mvn -pl ai4j-cli -am -Dtest=Ai4jCliTest -DfailIfNoTests=false -DskipTests=false test` passed with 29 tests covering check pass/fail/no-enable/validation-fail branches; `mvn -DskipTests package` passed across 11 reactor projects; `npm run typecheck` and `npm run build` passed in `docs-site/`. | none | Next executable/docs-site surface change. |
