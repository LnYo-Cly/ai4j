...
| --- | --- | --- | --- | --- | --- | --- |
| PR to `dev` or `main` touching Java modules or root build files | RG-001, RG-002, RG-003, RG-004, RG-005, RG-006, RG-007 | `none` by default | PR | L1/L2 | Mirrors `.github/workflows/java-regression.yml`: Java 8 package smoke plus module test matrix. | project coordinator |
...
| any merge to `dev` or `main` | RG-001..RG-009 | only opt-in gates explicitly required by the merged task/release | merge-batch | L1/L2; L3-L5 only when approved | Branch integration should refresh the full deterministic control surface. | project coordinator |
