...
Additionally, the frontend canvas and the backend task API are usually deployed on different origins, so cross-origin browser calls to `/flowgram/**` require CORS to be allowed by the backend: add the editor's origin to `ai4j.flowgram.cors.allowed-origins` (empty by default, bound to `CorsProperties`). For the full field list, see [Spring Boot Configuration Reference](/docs/integrations/spring-boot/configuration-reference) §5 (`ai4j.flowgram.*` / CORS).
