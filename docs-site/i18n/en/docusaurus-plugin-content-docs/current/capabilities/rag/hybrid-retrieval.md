...
There is no universally better choice — it depends on your embedding model, your BM25 corpus distribution, and your recall set size. The recommendation is to use the NDCG metric from [RAG Evaluation](/docs/capabilities/rag/evaluation) to run an A/B comparison on the same batch of labeled queries, rather than picking by gut feel.
