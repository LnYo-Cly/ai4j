...
So the answer-quality score travels with the trace, and is available later in [Citations and Trace](/docs/capabilities/rag/citations-and-trace). This pins "retrieval citations" and "answer-quality judgment" onto the same trace, so audit and UI rendering can pick them up together.
...
- **Offline retrieval metrics** (recall/NDCG, and the split of labor with the judge): [RAG Evaluation](/docs/capabilities/rag/evaluation)
- **Judgments travel with the trace**: [Citations and Trace](/docs/capabilities/rag/citations-and-trace)
- **How retrieval itself works**: [Hybrid Retrieval](/docs/capabilities/rag/hybrid-retrieval)
