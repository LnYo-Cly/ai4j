...
This is not infallible auto-detection: authentication failures, recognizable modern JSON-RPC errors, or unexpected successful responses will not be downgraded. The deprecated HTTP+SSE transport must still be configured with `type: "sse"`. For the full behavior, see [Streamable HTTP](/docs/capabilities/mcp/streamable-http).
