...
Unlike the synchronous / streaming semantics of Chat and Image, both follow an async lifecycle of **submit task → poll status → pull result**, and the SDK does not build in auto-polling. The `SUNO` platform currently exists only for the Music surface; every other service is unsupported. See [Video interface](/docs/capabilities/media/video-generation) and [Music interface](/docs/capabilities/media/music-generation).
