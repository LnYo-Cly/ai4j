...
| --- | --- | --- |
| Just want to send the first message | [Model Access](/docs/core-sdk/model-access/overview) | How to choose between Chat, Responses, streaming, and multimodal |
| Want the model to call local capabilities | [Tools](/docs/core-sdk/tools/overview) | Function Tool, schema, execution model, and security boundaries |
| Want to give the model reusable instructions and flows | [Skills](/docs/core-sdk/skills/overview) | Skill files, discovery, loading, and the boundary versus Tool/MCP |
| Want to connect external tools or publish Java capabilities | [MCP](/docs/mcp/overview) | client, transport, gateway, server publish |
| Want to build conversational context | [Memory](/docs/core-sdk/memory/overview) | chat memory, session, and the boundary versus tools |
| Want to build a knowledge base or retrieval augmentation | [Search & RAG](/docs/core-sdk/search-and-rag/overview) | ingestion, chunk, embedding, vector, rerank, citation |
| Want to extend providers or services | [Extension](/docs/core-sdk/extension/overview) | provider, model, service, HTTP stack extension patterns |

If this is your first time, start with [Quickstart for Java](/docs/start-here/quickstart-java), then return to this page to choose a capability line.
...
A unified entry point does not mean all provider capabilities are identical. Different platforms vary in their support for Chat, Responses, Embedding, Rerank, Image, Audio, and Realtime; before using them, consult the [Platform and Service Matrix](/docs/core-sdk/platform-service-matrix).
...
The legacy `ai-basics/`, `core-sdk/chat/`, `core-sdk/responses/`, and `core-sdk/mcp/` directories still hold historical details, but the current official reading path is governed by the sidebar and the [Documentation Map](/docs/start-here/documentation-map).
