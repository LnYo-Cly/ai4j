...
Concurrency dispatch and connection pooling are not hard-coded; they are provided via SPI (see [Auto Configuration / OkHttp SPI extension points](/docs/spring-boot/auto-configuration#7-okhttp-spi-扩展点)):
...
`http-node.allow-private-network` defaults to `false`. HTTP node requests pass through `HttpNodeSsrfGuard` first, which blocks loopback, private network, link-local, and cloud metadata addresses. Set it to `true` explicitly only when you genuinely need to reach internal services. See [Built-in Nodes / SSRF protection](/docs/flowgram/built-in-nodes#ssrf-guard).
...
- First-time wiring: see [Quickstart for Spring Boot](/docs/start-here/quickstart-spring-boot)
- Relay platforms: see [OpenAI-compatible and TroveBox](/docs/start-here/openai-compatible-and-trovebox)
- Multi-instance entry point: see [Service Entry and Registry](/docs/core-sdk/service-entry-and-registry)
