...
Therefore, `isInitialized()` means the client is ready to call, and should not be read as "all transports have completed an initialization handshake". For protocol details and upgrade paths for modern HTTP, see [Streamable HTTP](/docs/mcp/streamable-http).
...
1. [MCP usage paths and scenario selection](/docs/mcp/use-cases-and-paths)
2. [MCP transport types in depth](/docs/mcp/transport-types)
3. [MCP Client integration (single-service mode)](/docs/mcp/client-integration)
4. [Tool exposure semantics and security boundary](/docs/mcp/tool-exposure-semantics)
5. [MCP Gateway management](/docs/mcp/gateway-management)
6. [Build and publish an MCP Server](/docs/mcp/build-your-mcp-server)
