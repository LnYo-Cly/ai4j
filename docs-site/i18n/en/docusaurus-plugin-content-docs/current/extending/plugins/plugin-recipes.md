...
If you do not yet know what a plugin package is, see [Plugin Packages](/docs/extending/plugins/plugin-packages) first. If you want to author a third-party plugin, see [Plugin Author Cookbook](/docs/extending/plugins/plugin-author-cookbook).
...
- Adding a model platform: see [Provider Extension](/docs/extending/code-level/provider-extension).
- Adding request fields to an existing provider: see [Model Extension](/docs/extending/code-level/model-extension).
- Adding a top-level SDK capability surface: see [Service Extension](/docs/extending/code-level/service-extension).
- Only adjusting the HTTP dispatcher or connection pool: see [SPI HTTP Stack](/docs/extending/code-level/spi-http-stack).
