...
| `ai4j-cli trust` | Manages the trust list for workspace hooks (see [Lifecycle Hooks and Workspace Trust](/docs/coding-agent/lifecycle-hooks)) |
...
| Flag | Default | Description |
| --- | --- | --- |
| `--approval <safe\|manual\|auto>` | safe | Approval mode; see [Tools and the approval mechanism](/docs/coding-agent/tools-and-approvals) |
...
Once a sandbox is bound, `bash action=exec` is routed to `SandboxSession.execute(...)`, and the result carries `executionEnvironment`, `sandboxSessionId`, and `sandboxProviderId`. For the full boundary, see [Sandbox Routing](/docs/coding-agent/sandbox-routing).
...
4. [Sessions, streaming, and processes](/docs/coding-agent/session-runtime)
5. [Lifecycle hooks and workspace trust](/docs/coding-agent/lifecycle-hooks)
