...
Unlike the [SubAgent tool-call RPC](/docs/agent/subagent-handoff-policy#0-通讯与并行模型先建立正确心智), Teams members are **not** in a tool-call relationship with each other; they collaborate through two shared components:
...
2. [SubAgent and Handoff Policy](/docs/agent/subagent-handoff-policy)
