...
| CubeSandbox | `cubesandbox` | CubeAPI control plane of the open-source TencentCloud/CubeSandbox + envd `process.Process/Start` data plane | [CubeSandbox Provider](/docs/agent/cubesandbox-provider) |
...
CubeSandbox's environment variables, supported `SandboxSpec.config` keys, protocol mapping table, envd port (default `49983`, some templates use `49999`), local and live verification commands, and current boundaries are all on the [CubeSandbox Provider](/docs/agent/cubesandbox-provider) page.
:::
