...
- `A2ASkill` describes an agent capability in an A2A AgentCard; it is not a local `SKILL.md` file. Use `A2ASkillMapper` to publish an authorized ai4j `SkillDescriptor` as an A2A skill on the AgentCard (see [A2A — Publish ai4j Skills](/docs/agent/a2a#publish-ai4j-skills-to-the-agentcard)).
...
3. [Loading and activation](/docs/core-sdk/skills/loading-and-activation) — three loading paths and the scoped executor
