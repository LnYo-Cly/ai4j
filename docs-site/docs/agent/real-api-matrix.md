...
| CLI `/sandbox` | Host / CLI 绑定中 | `/sandbox status`、`/sandbox attach`、`/sandbox disable` | 显示当前 sandbox binding；`cubesandbox`/`cube` 可 attach 已有 live CubeSandbox session，其它 provider 仍 metadata-only | [Command Reference](/docs/coding-agent/command-reference) |
...
AI4J 当前不内置官方云 runner、VM、Docker 或 K8s 平台；但已经提供 CubeSandbox adapter 作为第一个真实远端 sandbox provider。SDK 仍保持合同优先，其它 provider 由宿主、插件或业务系统实现。
