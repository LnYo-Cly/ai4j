...
[`StateGraphWorkflowTest`](https://github.com/LnYo-Cly/ai4j/blob/main/ai4j-agent/src/test/java/io/github/lnyocly/agent/StateGraphWorkflowTest.java) 用 `StaticNode`/`CounterNode`（无需密钥）演示条件分支 + 循环路由，是 §5/§6 的可执行佐证。天气场景的 live 双节点 workflow 见 [实战 cookbook](/docs/agent/orchestration/weather-workflow-cookbook)。
:::
