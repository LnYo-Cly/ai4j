...
这套循环要理解成“底层单轮 runtime 语义”：

- `BaseAgentRuntime` 看到“本轮没有 tool calls”就会结束当前 `run(...)`
- 它不负责任务级 auto-continue、compact-after-continue、blocked stop reason
- 这些更偏产品层的控制，当前放在 `ai4j-coding` 的 outer loop，而不是塞回 `BaseAgentRuntime`
