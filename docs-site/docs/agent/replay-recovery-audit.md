...
accumulates streamed text into `outputText` and keeps the latest non-null raw response in `outputs`,
so `NodeReplayer.replayModelMock(...)` can prefer the raw payload and fall back to assembled text.
