...
accumulates streamed text into `outputText` and keeps the latest raw response in `outputs`, so
`NodeReplayer.replayModelMock(...)` can preserve the raw payload and fall back to assembled text.
