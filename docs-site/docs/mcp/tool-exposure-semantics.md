...
这组入口已经足够说明 AI4J 为什么把 tool 暴露面收敛到调用点，而不是做默认全开。

## 7. 这一页的核心边界

- 普通 agent 场景关注的是“本次请求要开放什么”
- MCP server 发布场景关注的是“哪些本地能力对外可发布”
- 两种语义不能共用同一套默认值，否则安全边界会漂移

把这条边界讲清楚，后续做权限审批、白名单治理和回归审计时会轻很多。
