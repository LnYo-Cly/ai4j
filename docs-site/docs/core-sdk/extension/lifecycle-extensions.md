...
:::warning
`LifecycleHookRegistry` 和 `AgentLifecycleHook` 当前标注为 `@Experimental(since = "2.4.3")`，签名和行为可能在后续小版本调整。依赖时请 pin 死确切版本，详见 [Extension 总览 - 插件 SPI 的稳定性矩阵](/docs/core-sdk/extension/overview#32-插件-spi-的稳定性矩阵)。
:::
