...
- 自定义 `RagContextAssembler`，例如 `TokenAwareRagContextAssembler` 用 token budget 控制注入上下文长度
...
### 9.4 只用 `finalTopK` 控制上下文长度

`finalTopK` 控制的是最终 hit 数量，不是 token 数。企业 RAG 如果需要避免 context 超窗，应该显式把默认 assembler 换成 `TokenAwareRagContextAssembler`，给它一个实际可用的 context token budget。

### 9.5 把 citation 当成最终 answer grounding 证明
...
5. [Hybrid Retrieval](/docs/core-sdk/search-and-rag/hybrid-retrieval)
6. [Rerank](/docs/core-sdk/search-and-rag/rerank)
7. [Citations and Trace](/docs/core-sdk/search-and-rag/citations-and-trace)
8. [Online Search](/docs/core-sdk/search-and-rag/online-search)
