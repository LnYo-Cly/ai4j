...
这页现在更适合作为历史内容索引，而不是正式阅读入口。

推荐的阅读顺序是：

1. 先从当前 canonical 专题树进入对应模块
2. 只在需要追溯博客来源、迁移背景或旧路径映射时再回到这里

这样可以避免把“历史文章路径”和“当前文档主线”混成一套导航。

## 6. 迁移映射应保持什么规则

为了让这张映射表长期有用，建议持续遵守三条规则：

- 每篇旧文章至少映射到一个当前 canonical 页面
- 旧路径如果已经拆成多个能力面，要明确拆到哪些专题树
- 新的方案页或模块页稳定后，再回填到这里更新索引

这样它就能成为维护用索引，而不是一次性迁移备忘录。

## 7. 维护时优先更新什么

如果后续还要继续整理旧内容，建议优先更新：

- 高流量历史文章对应到的新 canonical 页面
- 已经拆分出多个专题树的旧路径映射
- 与版本升级、排障、案例仓库最相关的索引项

这样这张表会优先服务当前读者和维护者，而不是只保留历史记录。
