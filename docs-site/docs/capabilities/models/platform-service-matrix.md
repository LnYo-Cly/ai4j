...
当前只支持 `ANTHROPIC`（原生 `IMessagesService`）。它让本就说 Anthropic 方言的系统**原生 in / 原生 out** 接入（零 OpenAI 转换），并复用同一条 `IMessagesService` 打到 Claude 及合作厂家的 Anthropic 兼容入口（智谱 / MiniMax coding-plan）。`ANTHROPIC` 平台同时保留 `Chat` 入口（统一 `IChatService` 适配器，翻译 OpenAI 格式）。详见 [Messages（Anthropic 原生）](/docs/capabilities/models/messages)。
...
它们与 Chat/Image 的同步/流式语义不同，都是 **提交任务 → 轮询状态 → 拉取结果** 的异步生命周期，SDK 不内建自动轮询。`SUNO` 平台当前只为 Music 这一条面存在，其它 service 全部不支持。详见 [Video 接口](/docs/capabilities/media/video-generation) 与 [Music 接口](/docs/capabilities/media/music-generation)。
