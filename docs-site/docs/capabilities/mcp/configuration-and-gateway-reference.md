...
`McpGateway` 通过 `setConfigSource(...)` 绑定配置源后，`McpGatewayConfigSourceBinding` 会把上面三类事件翻译成运行时动作（见 [Gateway 管理 - 配置源热更新](/docs/capabilities/mcp/gateway-management)）：新增/更新 → 创建 client 并接入，移除 → 断开并下线。
