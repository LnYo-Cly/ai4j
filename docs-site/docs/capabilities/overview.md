...
| 目标 | 入口 | 你会先学到 |
| --- | --- | --- |
| 只想发第一条消息 | [Model Access](/docs/capabilities/models/overview) | Chat、Responses、streaming、多模态怎么选 |
| 想让模型调用本地能力 | [Tools](/docs/capabilities/tools/overview) | Function Tool、schema、执行模型和安全边界 |
| 想给模型可复用说明和流程 | [Skills](/docs/capabilities/skills/overview) | Skill 文件、发现、加载和与 Tool/MCP 的边界 |
| 想接外部工具或发布 Java 能力 | [MCP](/docs/capabilities/mcp/overview) | client、transport、gateway、server publish |
| 想做会话上下文 | [Memory](/docs/capabilities/chat-memory/overview) | chat memory、session、与 tool 的边界 |
...
统一入口不等于所有 provider 能力完全一致。不同平台对 Chat、Responses、Embedding、Rerank、Image、Audio、Realtime 的支持不同，使用前应查看 [Platform and Service Matrix](/docs/capabilities/models/platform-service-matrix)。
