...
要换实现，在自己的 JAR 里放 `META-INF/services/io.github.lnyocly.ai4j.network.DispatcherProvider`（或 `...ConnectionPoolProvider`），写入实现类全名即可，无需改 starter 或业务 Bean。超时、代理、SSL 等可调参数仍由 `ai.okhttp.*` 控制（见 [Configuration Reference §4](/docs/integrations/spring-boot/configuration-reference#4-aiokhttp-的位置)）。
