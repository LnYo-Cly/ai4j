...
`ai4j-spring-boot-starter` 用来把 Core SDK 接入 Spring Boot 的配置、Bean 生命周期和业务分层。它不是一套新的 AI 实现，也不是必须先使用的入口；如果你不是 Spring 项目，直接从 [Core SDK](/docs/capabilities/overview) 开始即可。
...
| 你想扩展什么 | 从这里开始 |
| --- | --- |
| 覆盖默认 Bean | [Bean Extension](/docs/integrations/spring-boot/bean-extension) |
| 查看配置项 | [Configuration Reference](/docs/integrations/spring-boot/configuration-reference) |
| 组织常见业务写法 | [Common Patterns](/docs/integrations/spring-boot/common-patterns) |
| 回到模型和 Tool 语义 | [Core SDK](/docs/capabilities/overview) |
| 做 Spring 场景方案 | [Solutions](/docs/integrations/solutions/overview) |
