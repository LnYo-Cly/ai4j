...
- [Tool](/docs/capabilities/tools/overview)：本地函数声明、执行模型和安全边界。
- [Skill](/docs/capabilities/skills/overview)：模型可读取的说明、模板和任务资产。
- [MCP](/docs/capabilities/mcp/overview)：协议化接入外部工具和服务。

这种分层对小项目和长期项目都重要。小项目能少绕路，长期项目能避免后续重构时边界失控。

### 4. RAG 和检索链路可逐步引入

AI4J 把 [Search & RAG](/docs/capabilities/rag/overview) 放在 Core SDK 主线里，
