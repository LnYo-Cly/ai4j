...
`attach` 当前是 **metadata-only**：它让 CLI 能把 provider/session/workspace 摘要传到 `CodingAgentBuilder.sandbox(...)`，并让后续 `bash action=exec` 进入 `SandboxSession` 路由；但因为通用 `SandboxProvider.attachSession(sessionId)` / provider bridge 还没有落地，执行命令会明确失败：
...
```text
- sandbox=attached(cubesandbox/sbx_123, workspace=project-a, metadata-only)
```
...
| real provider attach/resume | 尚未实现通用 provider bridge |
| CubeSandbox / Docker / E2B / K8s provider | 由后续插件或业务方实现 |
...
5. provider bridge：真实 `attach/resume/create/list/destroy/logs`。
...
```bash
mvn -pl ai4j-cli -am "-Dtest=SlashCommandControllerTest,CodingCliSessionRunnerArgumentParsingTest,DefaultCodingCliAgentFactoryTest,CliAttachedSandboxSessionTest" -DskipTests=false -DfailIfNoTests=false test
```
