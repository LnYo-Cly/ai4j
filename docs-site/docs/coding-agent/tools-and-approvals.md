...
### 11.1 用 `/permissions` 查看当前审批边界

CLI / TUI / ACP 现在都可以通过 `/permissions` 或 `/permissions status` 查看当前审批摘要。它会显示：

- 当前 `approvalMode`；
- 配置来源提示：`--approval`、`AI4J_APPROVAL`、`ai4j.approval` 或默认值；
- 当前工具门禁行为摘要；
- ACP 下是否会通过 `session/request_permission` 请求宿主确认；
- sandbox 与审批的关系说明。

这个命令是只读诊断入口，不会修改审批模式，也不会输出 raw tool input、prompt、provider key 或工具输出全文。它适合在执行危险工具前确认“当前到底是 auto、safe 还是 manual”。
...
4. [Sandbox Routing](/docs/coding-agent/sandbox-routing)
