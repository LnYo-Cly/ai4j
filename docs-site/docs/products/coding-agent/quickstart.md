...
quickstart 只关心前三类**会话型入口**（`code`/`tui`/`acp`）；`run`/`extension`/`trust` 是单次工具型子命令，完整说明见 [命令参考 §7](/docs/products/coding-agent/command-reference)。
...
- 会话型运行入口是 `code`、`tui`、`acp`；此外 `ai4j-cli` 还有 `run`、`extension`、`trust` 三个单次工具型子命令（见 [命令参考](/docs/products/coding-agent/command-reference)）
...
4. [Tools 与审批机制](/docs/products/coding-agent/tools-and-approvals)
5. [MCP 对接](/docs/products/coding-agent/mcp-integration)
6. [ACP 集成](/docs/products/coding-agent/acp-integration)
