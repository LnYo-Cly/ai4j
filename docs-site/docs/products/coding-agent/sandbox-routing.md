...
CLI/TUI 内的 `/sandbox` 状态展示与切换**已经落地**（解析器 `CliSandboxCommand`），不再是规划中能力。可在会话内用 `/sandbox status`、`/sandbox enable <provider>`、`/sandbox attach <provider> <id>`、`/sandbox disable` 管理当前 session 的 sandbox 绑定，命令细节见 [命令参考](/docs/products/coding-agent/command-reference)。
:::
