...
final class TranscriptPrinter {

    private final TerminalIO terminal;
    private boolean printedBlock;

    TranscriptPrinter(TerminalIO terminal) {
        this.terminal = terminal;
    }

    synchronized void printBlock(List<String> rawLines) {
    ...
    synchronized void beginStreamingBlock() {
        printedBlock = true;
    }

    synchronized void printSectionBreak() {
        if (!printedBlock) {
            return;
        }
        terminal.println("");
    ...
    synchronized void resetPrintedBlock() {
        printedBlock = false;
    }
