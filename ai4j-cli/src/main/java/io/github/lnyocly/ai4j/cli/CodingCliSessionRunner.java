...
public class CodingCliSessionRunner {
    ...
    private DispatchResult dispatchInteractiveInput(ManagedCodingSession session, String input) throws Exception {
        ...
        if ("/stream".equalsIgnoreCase(normalized) || normalized.startsWith("/stream ")) {
            handleStreamCommand(extractCommandArgument(normalized));
            renderTui(session);
            return DispatchResult.stay(session);
        }
    ...
    private void runTurn(ManagedCodingSession session, String input) throws Exception {
        ...
        CliAgentListener listener = new CliAgentListener(session, turnId);
        try {
            session.getSession().runStream(CodingAgentRequest.builder().input(input).build(), listener);
    ...
    private void printSessionHelp() {
        ...
        builder.append("  /stream [on|off]  Show or switch transcript streaming\n");
        ...
        builder.append("TUI keys: / opens command list, Tab accepts completion, Ctrl+P palette, Ctrl+R replay, Enter submit, Esc clear input.\n");
    ...
    private ManagedCodingSession switchSessionRuntime(ManagedCodingSession session, CodeCommandOptions nextOptions) throws Exception {
        ...
        CodingCliAgentFactory.PreparedCodingAgent prepared = agentFactory.prepare(nextOptions, terminal, interactionState);
    ...
    private void printAutoCompactOutcome(ManagedCodingSession session, String turnId) {
        CodingSessionCompactResult result = session.getSession().drainLastAutoCompactResult();
        if (result != null) {
            printCompactResult(result);
            appendCompactEvent(session, result, turnId);
        }
        Exception error = session.getSession().drainLastAutoCompactError();
        if (error != null) {
    ...
    private List<String> renderCommandLines(List<CustomCommandTemplate> commands) {
        List<String> lines = new ArrayList<String>();
        lines.add("/help /status /session /theme /save /providers /provider /model");
        lines.add("/provider use|save|add|edit|default|remove");
        lines.add("/commands /palette /cmd <name> /sessions /history /tree /events /replay /compacts /checkpoint");
    ...
    private void handleStreamCommand(String argument) {
    ...
    private String renderStreamOutput() {
        ...
        builder.append("- behavior=").append(streamEnabled
                ? "assistant and reasoning text stream incrementally into the transcript"
                : "assistant and reasoning text render as completed blocks");
        return builder.toString().trim();
    }
    ...
    private final class CliAgentListener implements AgentListener {
        ...
        private CliAgentListener(ManagedCodingSession session, String turnId) {
            this.session = session;
            this.turnId = turnId;
        }
