...
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-06-10 local and remote pass | `npm run typecheck` and `npm run build` passed with the 8GB Node heap baked into package scripts; GitHub Actions docs-build run `27220942110` and docs-pages run `27220942127` passed on `main@0df7094` using the same typecheck/build entrypoints |
| RG-009 | 🟢 | FlowGram webapp demo build and test | `npm run test`, `npm run lint`, `npm run ts-check`, then `npm run build` in `ai4j-flowgram-webapp-demo/` | touched-surface, PR, merge-batch | L2 local_smoke | 2026-06-10 local pass | `npm test` now runs deterministic backend workflow normalization tests before lint/type/build; local `npm run test`, `npm run lint`, `npm run ts-check`, and `npm run build` passed. CI now runs `npm test` before the existing lint/type/build baseline; remote evidence is pending push for the R-003 implementation commit |
...
| R-003 | FlowGram webapp demo | resolved 2026-06-10: frontend `test`, `test:cov`, and `watch` scripts are no longer placeholders; `npm test` runs backend workflow normalization/serialization tests and CI runs it before lint/type/build. Remote workflow evidence is pending push for the implementation commit | P2 | 2026-04-26 |
