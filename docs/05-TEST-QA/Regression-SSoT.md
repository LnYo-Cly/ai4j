...
| RG-007 | 🟢 | monorepo package build | `mvn -DskipTests package` | PR, merge-batch, shared build change | L2 local_smoke | 2026-07-06 pass, 11 reactor projects | cross-module packaging and dependency alignment across root plus 10 modules, including extension API, official Ask User plugin, core SDK, agent, coding runtime, CLI, starters, FlowGram demo, BOM, and 2.4.0 plugin artifact/version alignment |
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-07-06 pass, `npm ci` + typecheck/build | `npm run build` first failed because this fresh worktree lacked ignored `docs-site/node_modules`; after `npm ci`, `npm run build` generated `docs-site/build` and `npm run typecheck` passed. Current docs now cover ask-user permission as host-policy metadata, plugin package permission boundaries, CLI lifecycle hook inspection, replay recovery outputText/raw-response split, `TOOL_ERROR` errorType, and 2.4.0 release/artifact references |
