...
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-06-20 P4 docs build pass | `npm --prefix docs-site run build` passed after local ignored dependency install in `.worktrees/feature/cli-sandbox-commands`; Coding Agent Sandbox Routing docs now cover P3 `CodingAgentBuilder.sandbox(SandboxSession)` routing and P4 `/sandbox status|attach|disable` CLI behavior, including metadata-only attach and no host fallback when no provider bridge exists; Command Reference and Agent SDK Roadmap link the P4 CLI/TUI sandbox commands without claiming real provider creation/authentication; prior docs-build run `27220942110` and docs-pages run `27220942127` passed on `main@0df7094` using the same typecheck/build entrypoints |
