...
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-07-05 RAG query planner strategy prompt docs pass | `npm run typecheck` and `npm run build` passed in `docs-site/`; docs now explain `RagQueryPlanner`, `ModelRagQueryPlanner` default rewrite-only behavior, explicit multi-query/HyDE/step-back strategy prompts, planner fallback, and the distinction from `HybridRetriever` and rerank |
