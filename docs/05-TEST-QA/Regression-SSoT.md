...
| RG-002 | 🟢 | agent runtime module | `mvn -pl ai4j-agent -am -DskipTests=false test` | touched-surface, PR, merge-batch | L1 tests | 2026-06-20 pass | agent runtime, workflow, memory, trace, subagent/team orchestration, extension lifecycle hook dispatch, tool approval / permission policy, and Agent Blueprint YAML loader / validator; `mvn -pl ai4j-agent -am "-Dtest=AgentBlueprintLoaderValidatorTest" -DskipTests=false -DfailIfNoTests=false test` passed with 9 P1-A blueprint tests; `mvn -pl ai4j-agent -am -DskipTests=false test` passed with extension API 25 tests, core 103 tests, and agent 103 tests; docs-site build result is recorded in SRB-052 |
