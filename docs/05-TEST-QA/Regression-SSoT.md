...
| RG-004 | 🟢 | CLI/TUI/ACP host | `mvn -pl ai4j-cli -am -DskipTests=false test` | touched-surface, PR, merge-batch | L1 tests | 2026-06-20 P4 broad pass, 289 CLI tests | terminal host, session runtime, ACP, rendering, provider/model command behavior with fake/local clients; includes P1-C `ai4j-cli run <agent.yaml>` Blueprint runner and P4 `/sandbox` CLI/TUI commands. P4 targeted `mvn -pl ai4j-cli -am "-Dtest=SlashCommandControllerTest,CodingCliSessionRunnerArgumentParsingTest,DefaultCodingCliAgentFactoryTest,CliAttachedSandboxSessionTest" -DskipTests=false -DfailIfNoTests=false test` passed with 60 tests; broad `mvn -pl ai4j-cli -am -DskipTests=false test` passed with extension API 25, core 103, agent 119, coding 61, and CLI 289 tests; metadata-only attached sandbox execution is covered by `CliAttachedSandboxSessionTest` and fails loudly instead of falling back to host execution |
...
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-06-20 P4 docs build pass | `npm --prefix docs-site run build` passed in `.worktrees/feature/memory-compact-session-api-polish`; Memory/Compact docs now document real `SessionCompactPlan` / `SessionCompactReport` APIs. Prior P4 build covered Coding Agent Sandbox Routing and `/sandbox status|attach|disable` CLI behavior without claiming real provider creation/authentication; prior docs-build run `27220942110` and docs-pages run `27220942127` passed on `main@0df7094` using the same typecheck/build entrypoints |
