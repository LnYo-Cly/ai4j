...
| RG-001 | 🟢 | core SDK module | `mvn -pl ai4j -am -DskipTests=false test` | touched-surface, PR, merge-batch | L1 tests | 2026-07-07 RAG token encoding fallback pass, 149 tests | core provider adapters, RAG, MCP, vector, realtime, agentflow contract tests; includes `FirstChatCopyableCodeTest`, RAG query planning/retrieval tests, online RAG judge coverage via `RagOnlineEvaluatorTest`, content-hash incremental ingestion coverage, and `TokenAwareRagContextAssemblerTest` for context token budget, first-hit truncation, unknown-model fallback, explicit `EncodingType` override, and default assembler compatibility; `TikTokensUtil` now asks jtokkit's string model registry on cache miss before failing; provider-dependent tests are excluded from default runs by `LiveProviderTest` category |
...
| RG-005 | 🟢 | Spring Boot starter | `mvn -pl ai4j-spring-boot-starter -am -DskipTests=false test` | touched-surface, PR, merge-batch | L1 tests | 2026-07-06 RAG vector lookup binding pass, 10 starter tests plus upstream gates | auto-configuration and config binding; includes first-chat, AgentFlow, extension resource activation config, Qdrant `scroll`, Milvus `query`, and Redis default `contentHash` tag binding for incremental RAG ingestion |
...
| RG-007 | 🟢 | monorepo package build | `mvn -DskipTests package` | PR, merge-batch, shared build change | L2 local_smoke | 2026-07-07 RAG token encoding fallback pass, 11 reactor projects | cross-module packaging and dependency alignment across root plus 10 modules, including extension API, official Ask User plugin, core SDK, agent, coding runtime, CLI, starters, FlowGram demo, and BOM; current RAG token task kept Java 8 source/package compatibility while adding explicit encoding override and safer model-name fallback |
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-07-07 RAG token encoding fallback docs pass, `npm ci` + typecheck/build | Fresh worktree lacked ignored `docs-site/node_modules`, so `npm ci` restored local dependencies; `npm run typecheck` and `npm run build` then passed and generated `docs-site/build`. Current RAG docs explain optional `TokenAwareRagContextAssembler`, model-name-first token counting, fallback estimates for unknown models, explicit `EncodingType` override, and Spring Bean override usage |
