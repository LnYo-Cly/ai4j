...
| RG-001 | 🟢 | core SDK module | `mvn -pl ai4j -am -DskipTests=false test` | touched-surface, PR, merge-batch | L1 tests | 2026-07-06 RAG token-aware context pass, 145 tests | core provider adapters, RAG, MCP, vector, realtime, agentflow contract tests; includes `FirstChatCopyableCodeTest`, RAG query planning/retrieval tests, online RAG judge coverage via `RagOnlineEvaluatorTest`, content-hash incremental ingestion coverage, and `TokenAwareRagContextAssemblerTest` for context token budget, first-hit truncation, unknown-model fallback, and default assembler compatibility; provider-dependent tests are excluded from default runs by `LiveProviderTest` category |
...
| RG-007 | 🟢 | monorepo package build | `mvn -DskipTests package` | PR, merge-batch, shared build change | L2 local_smoke | 2026-07-06 RAG token-aware context pass, 11 reactor projects | cross-module packaging and dependency alignment across root plus 10 modules, including extension API, official Ask User plugin, core SDK, agent, coding runtime, CLI, starters, FlowGram demo, and BOM; current RAG context task kept Java 8 source/package compatibility while adding optional token-aware assembly |
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-07-06 RAG token-aware context docs pass, `npm ci` + typecheck/build | Fresh worktree lacked ignored `docs-site/node_modules`, so `npm ci` restored local dependencies; `npm run typecheck` and `npm run build` then passed and generated `docs-site/build`. Current RAG docs explain optional `TokenAwareRagContextAssembler`, `finalTopK` versus token budget, and Spring Bean override usage |
