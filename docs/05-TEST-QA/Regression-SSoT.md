...
| RG-003 | 🟡 | coding runtime module | `mvn -pl ai4j-coding -am -DskipTests=false test` | touched-surface, PR, merge-batch | L1 tests | 2026-06-09 targeted pass / full gate blocked by upstream R-008 | `CodingAgentBuilderTest` covers extension tool execution; `CodingSkillSupportTest` passed 3 tests including extension Skill / Prompt projection into Coding Agent prompt and read-only resource roots; full `-am` evidence remains blocked by upstream RG-002/R-008 |
...
| RG-007 | 🟢 | monorepo package build | `mvn -DskipTests package` | PR, merge-batch, shared build change | L2 local_smoke | 2026-06-09 pass | cross-module packaging and dependency alignment across 10 reactor modules; passed after CLI extension command execution wiring |
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-06-09 pass | `NODE_OPTIONS=--max-old-space-size=8192 npm run typecheck` and `NODE_OPTIONS=--max-old-space-size=8192 npm run build` passed for CLI extension command execution documentation updates |
