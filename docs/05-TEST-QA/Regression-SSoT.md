...
| RG-007 | 🟢 | monorepo package build | `mvn -DskipTests package` | PR, merge-batch, shared build change | L2 local_smoke | 2026-07-05 HybridRetriever best-effort fallback pass, 11 reactor projects | cross-module packaging and dependency alignment across 11 reactor projects: root plus 10 modules, including extension activation plan API, CLI extension UX, Spring starter binding, official Ask User plugin module, F-041 `extension check` gate wiring, OpenAI-compatible video service, Suno music service, RAG query planner per-strategy prompt compile/package compatibility, and HybridRetriever fallback compile/package compatibility |
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-07-05 HybridRetriever best-effort fallback docs pass | `npm run typecheck` and `npm run build` passed in `docs-site/` after local ignored `node_modules` restore with `npm ci`; docs now explain default dense retrieval, BM25 corpus usage, Dense + BM25 `HybridRetriever` wiring, Query Planning multiplication cost, HybridRetriever best-effort fallback behavior, and links to the full hybrid retrieval + rerank workflow |
