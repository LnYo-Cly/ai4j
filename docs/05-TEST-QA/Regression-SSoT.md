...
| RG-004 | 🟢 | CLI/TUI/ACP host | `mvn -pl ai4j-cli -am -DskipTests=false test` | touched-surface, PR, merge-batch | L1 tests | 2026-06-20 P1-C broad pass, 283 CLI tests | terminal host, session runtime, ACP, rendering, provider/model command behavior with fake/local clients; includes P1-C `ai4j-cli run <agent.yaml>` Blueprint runner, host-supplied model client path, profile resolution guard, sandbox declaration guard, validation error rendering, and top-level `run --help`; targeted `mvn -pl ai4j-cli -am "-Dtest=AgentBlueprintRunCommandTest,Ai4jCliTest" -DskipTests=false -DfailIfNoTests=false test` passed with 35 tests; broad `mvn -pl ai4j-cli -am -DskipTests=false test` passed with extension API 25, core 103, agent 111, coding 59, cli 283 tests |
...
| RG-008 | 🟢 | docs-site build | `npm run typecheck`, then `npm run build` in `docs-site/` | touched-surface, docs PR/push, merge-batch | L2 local_smoke | 2026-06-20 P3 docs build pass | `npm --prefix docs-site run build` passed after local ignored dependency install in `.worktrees/feature/coding-sandbox-routing`; Coding Agent Sandbox Routing docs now cover `CodingAgentBuilder.sandbox(SandboxSession)`, `bash exec` routing through `SandboxShellCommandExecutor`, local fallback semantics, non-sensitive session binding, and not-yet-routed file/patch/browser boundaries; Agent Sandbox SPI docs cover provider/session/spec/command/result/artifact/event contracts, AgentSession sandbox binding, fake provider testing, permission policy boundary, Blueprint relation, and coding routing next step; Agent Blueprint docs cover P1-C CLI run usage, profile missing-fail guard, token boundary, and sandbox declaration boundary; prior docs-build run `27220942110` and docs-pages run `27220942127` passed on `main@0df7094` using the same typecheck/build entrypoints |
